...
def ask_ai(ctx, q):
    try:
        ...
        Analyze the provided health matrix (Latest data at the top).
        
        REQUIRED ANALYSIS:
        1. BODY COMPOSITION: How Calories/Steps impact Weight and Fat%.
        2. MUSCLE MASS: Calculate Muscle = Weight * (1 - (Fat% / 100)). Use Protein logs to judge if intake is sufficient.
        3. SLEEP: Correlate activity and macros with the Sleep Score.
        
        DATASET:
        {ctx}
        
        OUTPUT: Provide a technical report in Pure White text. Finish with a 3-point 'LEVEL UP' action plan.
        """
...
if st.session_state.tk:
    ...
    if st.sidebar.button("📊 ANALYSE TRENDS"):
        st.session_state.ms.append({"role": "user", "content": "Analyze my Body Comp, Muscle, and Sleep trends using all available metrics."})
    ...
    if not st.session_state.cached_data:
        st.info("Performance matrix ready for high-fidelity sync.")
        if st.button("🔄 SYNC & WEAVE PERFORMANCE DATA"):
            with st.status("Assembling 180-day Master Table...", expanded=True) as status:
                h = {"Authorization": f"Bearer {st.session_state.tk}"}
                try:
                    # A. PULL CORE TIME SERIES (6 Months)
                    ...
                    # C. DATA FOUNDRY (Key-Aggressive Mapping)
                    master = {}
                    def map_data(d_list, label):
                        for x in d_list:
                            d = x.get('dateTime') or x.get('date')
                            if d:
                                if d not in master: master[d] = {"s":0,"w":0,"f":0,"co":0,"in":0,"p":0,"c":0,"fat_g":0,"score":0}
                                # We check 'value' first, then common Fitbit keys
                                master[d][label] = x.get('value') or x.get('weight') or x.get('fat') or 0

                    map_data(s_raw, 's'); map_data(w_raw, 'w'); map_data(f_raw, 'f'); map_data(co_raw, 'co')
                    
                    for d_key, m_val in macros.items():
                        if d_key in master:
                            master[d_key].update({"in": m_val.get('calories', 0), "p": m_val.get('protein', 0), "c": m_val.get('carbs', 0), "fat_g": m_val.get('fat', 0)})
                    ...
                    for d in sorted(master.keys(), reverse=True):
                        v = master[d]
                        rows.append(f"{d},{v['s']},{v['w']},{v['f']},{v['co']},{v['in']},{v['p']},{v['c']},{v['fat_g']},{v['score']}")
    ...
    if st.session_state.cached_data:
        # DATA PROOF BOX (Let's see the numbers!)
        with st.expander("👁️ PREVIEW WEAVED DATA (Verify your numbers here)"):
            st.text(st.session_state.cached_data[:800]) # Shows header + first few rows
